...
# \
#
# \brief Get the landing page of published YoDa metadata
#
# \param[in] rodsZone
...
def jsonLandingPage(callback, rodsZone, vaultMetadataJsonPath):

    # Landing page creation is part of the publication proces
    # Read yoda-metadata.json from vault
    metadataDict = getMetadaJsonDict(callback, vaultMetadataJsonPath)


    # Load the jinja template file depending on category and version


    landingpage_template_path = '/' + rodsZone + '/schemas/templates/landingpage.html'
    ...
    msg = tm.render(title=title, description=description, disciplines=disciplines, version=version, language=language, tags=tags, creators=creators, contributors=contributors, publication_date=publication_date,
